...
def get_timewarrior_entries(start_date=None, end_date=None):
    ...
    if end_date:
        # Add two days to end_date to account for timezone differences
        # and ensure we capture entries that may span across UTC day boundaries
        try:
            end_dt = datetime.strptime(end_date, '%Y-%m-%d')
            inclusive_end = (end_dt + timedelta(days=2)).strftime('%Y-%m-%d')
